...
public class MonthlyPanel extends JPanel{
	...
	/**
	 * update the panel's entire display this method is to be called any time the
	 * appointment set is changed. this method is also to be called any time the
	 * view is changed
	 */
	public void update(Set<RefAppointment> as, Date cd, RefAppointment ca){
		...
		for(int i = 0; i < blocks.size(); i++){ // for each day in the month
			...
			dayBox.setEnabled(
					tmpCal.get(Calendar.MONTH) == curcal.get(Calendar.MONTH));
