...
public class DailyPanel extends JPanel{
	...
	private final WeeklyDayBlock block;

	private final Hashtable<Integer, JLabel> daysOfWeek;

	/**
	 * This is the constructor of the class it creates all of the weekDayBlocks
	 * that are associated with this class and sets the currently selected date and
	 * creates a weekRange around that date
	 *
	...
	public void update(Set<RefAppointment> refSet, Date selectedDate,
					   RefAppointment selectedAppointment){
		Calendar curcal = Calendar.getInstance();
		curcal.setTime(selectedDate);

		for(int key : daysOfWeek.keySet()){
			daysOfWeek.get(key).setEnabled(
					curcal.get(Calendar.DAY_OF_WEEK) == key);
		}
	...
	/**
	 * This method adds appointment listeners to all of the dayblocks for this
	 * week
	 *
